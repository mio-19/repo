...
lib.extendMkDerivation {
  ...
  extendDrvArgs =
    ...
    in
    lib.optionalAttrs (offlineRepo != null) {
      inherit (offlineRepo) gradleInitScript;
    }
    // lib.optionalAttrs (!(args ? dontStrip)) {
      dontStrip = true;
    }
    // lib.optionalAttrs (!(args ? gradleSetupHook)) {
      gradleSetupHook =
    ...
    // lib.optionalAttrs (!(args ? gradleFlags)) {
      gradleFlags = [
        "--console=plain"
      ]
    ...
    // {
      ...
      nativeBuildInputs = nativeBuildInputs ++ [ finalAttrs.gradleSetupHook ];

      passthru = lib.optionalAttrs (offlineRepo != null) { inherit offlineRepo; } // passthru;
    };
}
