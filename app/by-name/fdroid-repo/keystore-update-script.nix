...
let
  mkFdroidKeystoreUpdateScript =
    ...
    writeShellScriptBin name ''
      ...
      usage() {
        echo "Usage: ${name} <keystore> [--ks-pass <pass>] [--alias <appid|alias>]..."
        echo ""
        echo "Adds missing key aliases to an existing JKS/PKCS12 keystore."
        echo "By default, aliases are auto-discovered from ${repoPath}/unsigned APKs."
        echo "Options:"
      ...
      if [[ "''${#aliases[@]}" -eq 0 ]]; then
        if [[ ! -d "${repoPath}/unsigned" ]]; then
          echo "Expected unsigned APK directory in ${repoPath}/unsigned" >&2
          exit 1
        fi
        shopt -s nullglob
        apk_files=("${repoPath}"/unsigned/*.apk)
        shopt -u nullglob
        if [[ "''${#apk_files[@]}" -eq 0 ]]; then
          echo "No APK files found in ${repoPath}/unsigned" >&2
          exit 1
        fi
...
in
{
  inherit mkFdroidKeystoreUpdateScript;
}
