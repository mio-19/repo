# based on https://github.com/tadfisher/gradle2nix/blob/v2/nix/build-maven-repo.nix commit 30cfe5889188524223364ee7919d94e83d6ee44a
{
  lib,
  stdenvNoCC,
  coreutils,
...
let
  ...
  readLockFile = lockFile: if isAttrsNotDer lockFile then lockFile else fromJSON (readFile lockFile);
...
in
{
  passthru.readLockFile = readLockFile;
  __functor =
    self:
    {
      # Path to the lockfile generated by gradle2nix (e.g. gradle.lock).
      lockFile ? null,
      dependencies ? readLockFile lockFile,
