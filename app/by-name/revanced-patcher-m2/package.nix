...
stdenv.mkDerivation (finalAttrs: {
  ...
  mitmCache = gradle.fetchDeps {
    pname = "revanced-patcher";
    pkg = finalAttrs.finalPackage;
    data = ./revanced-patcher_deps.json;
    silent = false;
    useBwrap = false;
  };
  ...
  postUnpack = ''
        ...
        cp -a ${revanced-apktool-m2}/* "$sourceRoot/.m2/repository/"
        chmod -R u+w "$sourceRoot/.m2/repository"
        cp -a ${revanced-multidexlib2-m2}/* "$sourceRoot/.m2/repository/"
        chmod -R u+w "$sourceRoot/.m2/repository"
        ...
        substituteInPlace "$sourceRoot/patcher/build.gradle.kts" \
          --replace-fail '            maven {
                    name = "githubPackages"
                    url = uri("https://maven.pkg.github.com/revanced/revanced-patcher")
                    credentials(PasswordCredentials::class)
  ...
  meta = with lib; {
    description = "ReVanced patcher artifacts published to a local Maven repository";
    homepage = "https://github.com/ReVanced/revanced-patcher";
