...
stdenv.mkDerivation {
  ...
  src = fetchFromGitHub {
    ...
    repo = "gradle";
    tag = "v1.12";
    hash = "sha256-Lqm7qbVdgB09UW9zrglTvVmUer/T0u6cGyQaLSxF7WM=";
  };

  nativeBuildInputs = [ jdk8_headless ];

  patches = [ ./gradle-1.12-direct-bootstrap.patch ];
  ...
  buildPhase = ''
    ...
    cp ${gradle_1_6}/libexec/gradle/lib/*.jar ${gradle_1_6}/libexec/gradle/lib/plugins/*.jar build/lib/
    ...
    for subproject in resources base-services base-services-groovy core core-impl launcher messaging native cli tooling-api wrapper; do
    ...
    buildTimestamp=20140429000000+0000
    buildNumber=direct
