...
let
  ...
  artifacts = [
    ...
    {
      repo = gradleRepo;
      path = "net/rubygrapefruit/native-platform/0.2/native-platform-0.2.jar";
      hash = "sha256-KUR+KgXjeBfx7x9cp0DjW8PEr3I4Guk5zDuXS6b7er0=";
    }
    {
...
stdenv.mkDerivation {
  ...
  src = fetchFromGitHub {
    ...
    repo = "gradle";
    tag = "v1.4";
    hash = "sha256-ylsZE35AJpZPEUhZIXDLCSEJt+XePPCR9kOdjKARpRc=";
  };

  nativeBuildInputs = [ jdk8_headless ];

  patches = [ ./gradle-1.4-direct-bootstrap.patch ];
  ...
  buildPhase = ''
    ...
    cp ${gradle_1_2}/libexec/gradle/lib/*.jar ${gradle_1_2}/libexec/gradle/lib/plugins/*.jar build/lib/
    ...
    buildTimestamp=20121207000000+0000
