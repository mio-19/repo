...
stdenv.mkDerivation (finalAttrs: {
  ...
  src = fetchFromGitHub {
    ...
    hash = "sha256-PpZvm+2GoB8qqLKV0pszZ1UaKVSBLQQqc3BsK1kQ5Qc=";
  };
  ...
  installPhase = ''
    ...
    mkdir -p api-classes jcl-classes jul-classes log4j-classes
    ...
    for artifactId in slf4j-api jcl-over-slf4j jul-to-slf4j log4j-over-slf4j; do
      classes_dir="$tmp/''${artifactId%%-*}-classes"
      case "$artifactId" in
