...
let
  appPackage =
    let
      ...
      # https://github.com/bitwarden/android/blob/v2026.6.1-bwpm/gradle/libs.versions.toml#L33 bitwardenSdk = "3.0.0-7536-159aed97"
      sdkSrc = fetchFromGitHub {
        owner = "bitwarden";
        repo = "sdk-internal";
        rev = "159aed97";
        hash = "sha256-EDgnfM//F05drC844XmHIo0ZdUE6T3Sc8fT+deb95E8=";
      };

      sdkSrcLock = fetchurl {
        url = "${sdkSrc.meta.homepage}/raw/${sdkSrc.rev}/Cargo.lock";
        hash = "sha256-AxE978Vq0SlUrsmOV/gbpfOvuknN+zYOzHNl3EBSrtI=";
      };
      ...
      sdkGeneratedJava = stdenv.mkDerivation {
        ...
        cargoDeps = rustPlatform.fetchCargoVendor {
          ...
          src = sdkSrc;
          cargoRoot = ".";
          hash = "sha256-E9w8Qq6iqect57G2lGx1l7U8PFLCuQWG26z05eSvfMU=";
        };
        nativeBuildInputs = [
    ...
    stdenv.mkDerivation (finalAttrs: {
      ...
      src = fetchFromGitHub {
        ...
        hash = "sha256-nkNZUFxSHn073HWRyujcyS6zv4q1a8mUtY1ydrbQvfM=";
      };

      gradleBuildTask = ":authenticator:assembleRelease";
      gradleUpdateTask = "resolveNixBootstrapDeps :authenticator:assembleRelease";

      mitmCache = gradle.fetchDeps {
        inherit (finalAttrs) pname;
        pkg = finalAttrs.finalPackage;
        data = ./bitwarden-authenticator_deps.json;
        silent = false;
        useBwrap = false;
      };
      ...
      installPhase = ''
        runHook preInstall
        install -Dm644 \
          authenticator/build/outputs/apk/release/com.bitwarden.authenticator.apk \
          "$out/bitwarden-authenticator.apk"
        runHook postInstall
      '';
...
mk-apk-package {
  inherit appPackage;
  mainApk = "bitwarden-authenticator.apk";
  signScriptName = "sign-bitwarden-authenticator";
  fdroid = {
    ...
    metadataYml = ''
      ...
      AutoName: Bitwarden Authenticator
      Summary: Open-source TOTP authenticator
      Description: |-
        Bitwarden Authenticator stores and generates two-factor
        authentication (TOTP) verification codes.

        This package builds Bitwarden Authenticator from source.
    '';
  };
}
