...
def is_stable_tag(tag, allow_prerelease=False):
    if allow_prerelease:
        return True
    tl = tag.lower()
    if 'dev' in tl or 'beta' in tl or 'alpha' in tl or 'rc' in tl or 'internal' in tl or 'closed' in tl or 'open' in tl:
        return False
...
def main():
    ...
    for d in dirs_to_check:
        ...
        for pkg in sorted(os.listdir(d)):
            ...
            if pkg in ['onlyoffice-documents', 'biliroaming', 'morphe-library-m2', 'npatch', 'npatch-clionly', 'revanced-apktool-m2', 'revanced-library-m2', 'revanced-multidexlib2-m2', 'revanced-patcher-m2', 'revanced-patches-gradle-plugin'] or pkg.startswith('morphe-patcher-src') or 'plugin' in pkg or 'library-m2' in pkg:
                continue
