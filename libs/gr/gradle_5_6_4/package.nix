...
let
  mkGradle' =
    ...
    let
      gradleRunner = gradle_5_5_1;
      gradleRunnerKotlinDeps = stdenv.mkDerivation (runnerAttrs: {
        pname = "gradle-5.6.4-runner-kotlin-deps";
        version = "1.3.31";
    ...
    stdenv.mkDerivation (finalAttrs: {
      ...
      patches = [
        ./bootstrap-gradle-5-5-runner.patch
        ./bootstrap-test-fixtures-dependencies.patch
        ./bootstrap-library-elements-attribute.patch
      ];
      patchFlags = [ "-p1" ];
      ...
      preBuild = ''
        ...
        mkdir -p build
        printf '%s\n' 'v5.6.4' > build/determined-commit-id.txt
        ...
        gradleRunnerClasspath="$gradleRunnerKotlinCache/kotlin-compiler-embeddable/1.3.31/kotlin-compiler-embeddable-1.3.31.jar:$gradleRunnerKotlinCache/kotlin-reflect/1.3.31/kotlin-reflect-1.3.31.jar:$gradleRunnerKotlinCache/kotlin-script-runtime/1.3.31/kotlin-script-runtime-1.3.31.jar:$gradleRunnerKotlinCache/kotlin-stdlib/1.3.31/kotlin-stdlib-1.3.31.jar:$gradleRunnerKotlinCache/kotlin-stdlib-common/1.3.31/kotlin-stdlib-common-1.3.31.jar:$gradleRunnerKotlinM2/intellij/deps/trove4j/1.0.20181211/trove4j-1.0.20181211.jar:$gradleRunnerKotlinM2/annotations/13.0/annotations-13.0.jar:$gradleRunnerCache/commons-logging/commons-logging/1.2/commons-logging-1.2.jar:${gradleRunner}/libexec/gradle/lib/plugins/commons-codec-1.11.jar:${gradleRunner}/libexec/gradle/lib/plugins/httpclient-4.5.6.jar:${gradleRunner}/libexec/gradle/lib/plugins/httpcore-4.4.10.jar:${gradleRunner}/libexec/gradle/lib/plugins/jsch-0.1.54.jar"
        export GRADLE_OPTS="-Xmx2048m -XX:MaxMetaspaceSize=768m -Dfile.encoding=UTF-8 -Dkotlin.compiler.execution.strategy=in-process -Dkotlin.daemon.enabled=false -Xbootclasspath/a:$gradleRunnerClasspath ''${GRADLE_OPTS:-}"
        cat > "$GRADLE_USER_HOME/gradle.properties" <<EOF
        org.gradle.daemon=false
        org.gradle.jvmargs=-Xmx2048m -XX:MaxMetaspaceSize=768m -Dfile.encoding=UTF-8 -Dkotlin.compiler.execution.strategy=in-process -Dkotlin.daemon.enabled=false -Xbootclasspath/a:$gradleRunnerClasspath
        ...
        gradle.projectsLoaded {
          rootProject.allprojects {
            repositories.all { repo ->
              if (repo.hasProperty('url') && repo.url != null && repo.url.toString() == 'http://ajax.googleapis.com/ajax/libs') {
                repo.url = uri('https://ajax.googleapis.com/ajax/libs')
              }
            }
            repositories {
              mavenCentral()
      ...
      postPatch = ''
        ...
        replaceIfPresent buildSrc/settings.gradle.kts \
          'enableFeaturePreview("GROOVY_COMPILATION_AVOIDANCE")' \
          '// GROOVY_COMPILATION_AVOIDANCE is unavailable in the bootstrap Gradle'
        replaceIfPresent subprojects/distributions/distributions.gradle \
          '    testImplementation(testFixtures(project(":core")))' \
          '    testImplementation(project(path: ":core", configuration: "testFixturesRuntimeElements"))'
        ...
        replaceIfPresent subprojects/docs/docs.gradle \
          'http://ajax.googleapis.com/ajax/libs' \
          'https://ajax.googleapis.com/ajax/libs'
        replaceIfPresent subprojects/reporting/reporting.gradle \
          'http://ajax.googleapis.com/ajax/libs' \
          'https://ajax.googleapis.com/ajax/libs'
        if [ -f buildSrc/subprojects/kotlin-dsl/src/main/kotlin/build/GradleApiParameterNamesTransform.kt ]; then
