...
let
  mkGradle' =
    ...
    let
      gradleRunner = gradle_5_4_1;
      gradleRunnerKotlinDeps = stdenv.mkDerivation (runnerAttrs: {
        pname = "gradle-5.5.1-runner-kotlin-deps";
        version = "1.3.21";
    ...
    stdenv.mkDerivation (finalAttrs: {
      ...
      src = fetchFromGitHub {
        ...
        repo = "gradle";
        tag = "v5.5.1";
        hash = "sha256-diEqvExKfHygZXuwshhkYdjbX02dnDupoLlvdXu/+YE=";
      };
      ...
      preBuild = ''
        ...
        export GRADLE_OPTS="-Xmx2560m -XX:MaxMetaspaceSize=1024m -Dfile.encoding=UTF-8 -Xbootclasspath/a:$gradleRunnerClasspath ''${GRADLE_OPTS:-}"
        cat > "$GRADLE_USER_HOME/gradle.properties" <<EOF
        org.gradle.daemon=false
        org.gradle.jvmargs=-Xmx2560m -XX:MaxMetaspaceSize=1024m -Dfile.encoding=UTF-8 -Xbootclasspath/a:$gradleRunnerClasspath
