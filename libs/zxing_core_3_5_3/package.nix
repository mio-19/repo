...
stdenv.mkDerivation (finalAttrs: {
  ...
  src = fetchFromGitHub {
    ...
    hash = "sha256-hMJTS9iRibrb7LrHIBREDWb+y7a3vFrCr8wHCQ9Poiw=";
  };
  ...
  installPhase = ''
    ...
    mkdir -p classes
    find "${finalAttrs.src}/core/src/main/java" -name '*.java' > sources.txt
    javac --release 8 -d classes @sources.txt

    (
      cd classes
      jar cf "$tmp/core-${finalAttrs.version}.jar" .
    )
