# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  axp_build = {
    pname = "axp_build";
    version = "adef6303e5cfce57f95822fdfe1db9d2fd5c87e1";
    src = fetchgit {
      url = "https://git.disroot.org/AXP.OS/build.git";
      rev = "adef6303e5cfce57f95822fdfe1db9d2fd5c87e1";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Gy6/zJfpjjvgvfQ9WuJDZ9SEEfZt2wuxQ81Qg8OFQWY=";
    };
    date = "2026-05-25";
  };
  axp_kernel_patches = {
    pname = "axp_kernel_patches";
    version = "a20109ae4aab35215e4acf38f08cb93aeab64aa4";
    src = fetchgit {
      url = "https://git.disroot.org/AXP.OS/kernel_patches.git";
      rev = "a20109ae4aab35215e4acf38f08cb93aeab64aa4";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-JKkPk0OQYqfr+D6vZWZVPus3dSZfRxuQSbuQZTuiuDM=";
    };
    date = "2026-09-01";
  };
  cola2261_device_dm3q = {
    pname = "cola2261_device_dm3q";
    version = "2f16cacd879edef6a551de89e449fe42449179db";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-cola2261/android_device_samsung_dm3q.git";
      rev = "2f16cacd879edef6a551de89e449fe42449179db";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-FARhyjqP+2bx81U+bgySl0PXIU8QVNQRgy3hW8NVYJ0=";
    };
    date = "2026-08-06";
  };
  cola2261_device_sm8550_common = {
    pname = "cola2261_device_sm8550_common";
    version = "36d871df9b672a47c849583894500cfaa7d838b7";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-cola2261/android_device_samsung_sm8550-common.git";
      rev = "36d871df9b672a47c849583894500cfaa7d838b7";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-FW4mRva1JTWp72efEZMGgArbx0KJRQ8+0YS5/pdg6wE=";
    };
    date = "2026-08-10";
  };
  cola2261_hardware_samsung = {
    pname = "cola2261_hardware_samsung";
    version = "4f3afca5e2af0b447b3efd3973ecb69c63a5f918";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-cola2261/android_hardware_samsung.git";
      rev = "4f3afca5e2af0b447b3efd3973ecb69c63a5f918";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-uQbbwazvQ0zs7JB1ZyzLyUMONzYlJXHWZE7gVvMhzjQ=";
    };
    date = "2025-12-08";
  };
  cola2261_kernel_sm8550 = {
    pname = "cola2261_kernel_sm8550";
    version = "c10438709d017451a5845217252b7cf3e97e0177";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-cola2261/android_kernel_samsung_sm8550.git";
      rev = "c10438709d017451a5845217252b7cf3e97e0177";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-/aMZ5PAkEA8enrJ+WxY/a1J2GsHOJ0NnLtiD/+pXQ7A=";
    };
    date = "2026-07-23";
  };
  cola2261_kernel_sm8550_modules = {
    pname = "cola2261_kernel_sm8550_modules";
    version = "2f3d47292a970a6e2a25cad8dab6daa4c019df0f";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-cola2261/android_kernel_samsung_sm8550-modules.git";
      rev = "2f3d47292a970a6e2a25cad8dab6daa4c019df0f";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-8DnvLYVdpGxfOQdEp6Li6p4zax+dzKqkKInGnBr7zTw=";
    };
    date = "2026-07-21";
  };
  cola2261_vendor_dm3q = {
    pname = "cola2261_vendor_dm3q";
    version = "6d148e37996cddcbe92d6d8133b12e617df75610";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-cola2261/proprietary_vendor_samsung_dm3q.git";
      rev = "6d148e37996cddcbe92d6d8133b12e617df75610";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Y+u8kBDYOP1oSGEmiilIrcFQbNZemvOw4XDfygksCzU=";
    };
    date = "2026-08-06";
  };
  cola2261_vendor_sm8550_common = {
    pname = "cola2261_vendor_sm8550_common";
    version = "3d9749aabb0089b54b7ffb69ff52cb0b02c280a2";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-cola2261/proprietary_vendor_samsung_sm8550-common.git";
      rev = "3d9749aabb0089b54b7ffb69ff52cb0b02c280a2";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-A9fMDG4abDmetxhAPk24XBrZlGcWTEo6wdZts902bEY=";
    };
    date = "2026-08-10";
  };
  derpfest16_device_oneplus_enchilada = {
    pname = "derpfest16_device_oneplus_enchilada";
    version = "0ffb6826a8e39b6208d03e430320bf1b782185f6";
    src = fetchgit {
      url = "https://github.com/ppanzenboeck/device_oneplus_enchilada.git";
      rev = "0ffb6826a8e39b6208d03e430320bf1b782185f6";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-wG8YT+g2o1Rb0bDrjJ1D9+gDTa6AsKmMZLhaE6iUseg=";
    };
    date = "2026-04-15";
  };
  derpfest16_device_oneplus_sdm845_common = {
    pname = "derpfest16_device_oneplus_sdm845_common";
    version = "279a14fbb48544071f2352b84eab511f377c1ea8";
    src = fetchgit {
      url = "https://github.com/ppanzenboeck/device_oneplus_sdm845-common.git";
      rev = "279a14fbb48544071f2352b84eab511f377c1ea8";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-uMcuA/xBEijfxSncLWEISlNl6dVooHnkvINmln4HqYU=";
    };
    date = "2026-03-17";
  };
  derpfest16_hardware_oneplus = {
    pname = "derpfest16_hardware_oneplus";
    version = "58032612ed86f943ac0fe927a833156d26fcbdf8";
    src = fetchgit {
      url = "https://github.com/ppanzenboeck/hardware_oneplus.git";
      rev = "58032612ed86f943ac0fe927a833156d26fcbdf8";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-2AaNAbTnNYBXq5tug3of0RZZngm8gTjFyhHMgjodT4Y=";
    };
    date = "2026-05-10";
  };
  derpfest16_kernel_oneplus_sdm845 = {
    pname = "derpfest16_kernel_oneplus_sdm845";
    version = "9b322418d63762b3bec0c824a656227a6607aa9d";
    src = fetchgit {
      url = "https://github.com/ppanzenboeck/kernel_oneplus_sdm845.git";
      rev = "9b322418d63762b3bec0c824a656227a6607aa9d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-3w7YBmhKSNsQFDrYNUtd/Uv6P5MPY6u/v4oS6RI2lNY=";
    };
    date = "2026-03-17";
  };
  derpfest_launcher3 = {
    pname = "derpfest_launcher3";
    version = "4ac0a7a518831fe131456cca7d243e7f86eaa7cf";
    src = fetchgit {
      url = "https://github.com/DerpFest-AOSP/android_packages_apps_DerpLauncher.git";
      rev = "4ac0a7a518831fe131456cca7d243e7f86eaa7cf";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-cL84fYW64suqm9bBDA/ekR+Xd4acpuUMNoE9DNFdn24=";
    };
    date = "2026-06-28";
  };
  evox_launcher3 = {
    pname = "evox_launcher3";
    version = "6f469e925d61c856360ee067cd29d5e136457151";
    src = fetchgit {
      url = "https://github.com/Evolution-X/packages_apps_Launcher3.git";
      rev = "6f469e925d61c856360ee067cd29d5e136457151";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-/ol/RbOUByAWOZ/II+4D6cFOloEUd96jg3bdT0dEr7o=";
    };
    date = "2026-02-03";
  };
  external_lxc = {
    pname = "external_lxc";
    version = "4e3a3630fff3dc04e0d4a761309f87f248e40b17";
    src = fetchgit {
      url = "https://github.com/Linux-on-droid/external_lxc.git";
      rev = "4e3a3630fff3dc04e0d4a761309f87f248e40b17";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-lh/YEh1ubAW51GKFZiraQZqbGGkdT6zuSVunDRAaKbE=";
    };
    date = "2024-11-24";
  };
  grapheneos_appstore = {
    pname = "grapheneos_appstore";
    version = "bdfeafd48ee39fd15df9150877ccd5bc09ae061e";
    src = fetchgit {
      url = "https://github.com/GrapheneOS/AppStore.git";
      rev = "bdfeafd48ee39fd15df9150877ccd5bc09ae061e";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-nJgDQ8MRYmn8D6ycr1bF9g8TT5+iDFn/6FEAm74qmTY=";
    };
    date = "2026-08-27";
  };
  grapheneos_camera = {
    pname = "grapheneos_camera";
    version = "dbceb4b6eb290afd84b134ca28d679fec82ee05f";
    src = fetchgit {
      url = "https://github.com/GrapheneOS/Camera.git";
      rev = "dbceb4b6eb290afd84b134ca28d679fec82ee05f";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-mZ2BUQVZKT7vptC3KvFZX7hh/260ZwtGazbXjyaNQzM=";
    };
    date = "2026-08-27";
  };
  grapheneos_info = {
    pname = "grapheneos_info";
    version = "f5216fe1c86163fde86ddbe7b1f4d0cab816f42b";
    src = fetchgit {
      url = "https://github.com/GrapheneOS/Info.git";
      rev = "f5216fe1c86163fde86ddbe7b1f4d0cab816f42b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-y9sLALWnMwvXyi505mvjQERILzFNYXDg/ZLCpwsFw04=";
    };
    date = "2026-08-27";
  };
  grapheneos_pdfviewer = {
    pname = "grapheneos_pdfviewer";
    version = "5442c78a0757bb57690df789b6f902917bd8069d";
    src = fetchgit {
      url = "https://github.com/GrapheneOS/PdfViewer.git";
      rev = "5442c78a0757bb57690df789b6f902917bd8069d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-TUzffIbH2q5lGdJ2gWo8kiIbjVBg7LpYFX5gz+L3E3Q=";
    };
    date = "2026-09-07";
  };
  grapheneos_vanadium = {
    pname = "grapheneos_vanadium";
    version = "670dd99e7ba5b79be895d4144209ea0bea0ec773";
    src = fetchgit {
      url = "https://gitlab.com/grapheneos/platform_external_vanadium.git";
      rev = "670dd99e7ba5b79be895d4144209ea0bea0ec773";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-sJ99VbcWOBc2PQND0QDwY47IrfZCBY7fOeUqeNvuMl0=";
    };
    date = "2026-06-17";
  };
  gta4xlwifi-evobka-kernel = {
    pname = "gta4xlwifi-evobka-kernel";
    version = "2c276961bbd7e6b8cad97219431182049bfc5e1c";
    src = fetchgit {
      url = "https://github.com/Evolution-X-Devices/kernel_samsung_gta4xl.git";
      rev = "2c276961bbd7e6b8cad97219431182049bfc5e1c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-GSBGZEQM7kVS7Xr5A8yjDs822wbqZ3VXpD7+bNDIXUw=";
    };
    date = "2025-09-05";
  };
  libhybris = {
    pname = "libhybris";
    version = "29e0fa6da0166261f6b9437582fe2d56bab8530c";
    src = fetchgit {
      url = "https://github.com/Linux-on-droid/libhybris.git";
      rev = "29e0fa6da0166261f6b9437582fe2d56bab8530c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-5Ji9R1M0PKNWb8mQyMESQAI2qRli2GMIIMMVDsjZ7NE=";
    };
    date = "2026-05-29";
  };
  lindroid_drm_loopback = {
    pname = "lindroid_drm_loopback";
    version = "d3b85f3251beae4bc8481538f37d13b7f30abde0";
    src = fetchgit {
      url = "https://github.com/Linux-on-droid/lindroid-drm-loopback.git";
      rev = "d3b85f3251beae4bc8481538f37d13b7f30abde0";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-mEJ3LY4lNoWLaFz9ANfRAzZpOaTJnkbZwBR0Oe1pyyg=";
    };
    date = "2026-06-24";
  };
  lineage_deskclock = {
    pname = "lineage_deskclock";
    version = "e7fd65ec8f5fb1496ee7ac85e70327f97689380c";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_packages_apps_DeskClock.git";
      rev = "e7fd65ec8f5fb1496ee7ac85e70327f97689380c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-9k6I9/mq7xDhmSg8b0kykVK+UjAfn/O+yzS8mHD2Fqw=";
    };
    date = "2026-09-01";
  };
  lineage_device_mainline_common = {
    pname = "lineage_device_mainline_common";
    version = "5c5fd330f4cc0d7214eca65032b03c252b008f82";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_device_mainline_common.git";
      rev = "5c5fd330f4cc0d7214eca65032b03c252b008f82";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-sD4VcyKiNBKGP2n1NjTMbabYVBZzg/WIDpVS+ZINGaY=";
    };
    date = "2026-08-15";
  };
  lineage_device_virt_common = {
    pname = "lineage_device_virt_common";
    version = "071865e9d0c202785c971e0d9ee114e36a514ad7";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_device_virt_virt-common.git";
      rev = "071865e9d0c202785c971e0d9ee114e36a514ad7";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-g5c3u7nWSTU6/HWOCIj6wOQxkJpffJ8MUGK4iVFLpZU=";
    };
    date = "2026-08-15";
  };
  lineage_device_virtio_arm64 = {
    pname = "lineage_device_virtio_arm64";
    version = "3346a2fb77da6c8a2728cca8313530ba74de3798";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_device_virt_virtio_arm64.git";
      rev = "3346a2fb77da6c8a2728cca8313530ba74de3798";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-GxorbnqWIDTWbWiI3lyAv3byhR9/1PUVvgZWL1XAO3A=";
    };
    date = "2025-12-11";
  };
  lineage_device_virtio_arm64only = {
    pname = "lineage_device_virtio_arm64only";
    version = "05609653c67fe93de3a7f0e115a1680355c9a8e5";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_device_virt_virtio_arm64only.git";
      rev = "05609653c67fe93de3a7f0e115a1680355c9a8e5";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-fZCK7Xq5nevpzSAp+cb16wTxwGZzgF3EfkX9ARktSF8=";
    };
    date = "2025-11-11";
  };
  lineage_device_virtio_common = {
    pname = "lineage_device_virtio_common";
    version = "f17927b21dd5eceaf85c8dd3f3e7f8e1a7d3d074";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_device_virt_virtio-common.git";
      rev = "f17927b21dd5eceaf85c8dd3f3e7f8e1a7d3d074";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-kzTG+JPMB1+AeVGd9npphkuKmhNJgD+/9V/itAimFj8=";
    };
    date = "2026-08-15";
  };
  lineage_exactcalculator = {
    pname = "lineage_exactcalculator";
    version = "af4a9397439223d0f572823d66c7985619da3e45";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_packages_apps_ExactCalculator.git";
      rev = "af4a9397439223d0f572823d66c7985619da3e45";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-8bBMsfspH0CxhdKaU+lijehwBUaPK0Zh+u5+bkh6BZA=";
    };
    date = "2026-06-08";
  };
  lineage_external_drm_hwcomposer_upstream = {
    pname = "lineage_external_drm_hwcomposer_upstream";
    version = "8976786556df09c42b4ad41dd93969c6792f324d";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_external_drm_hwcomposer-upstream.git";
      rev = "8976786556df09c42b4ad41dd93969c6792f324d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-e5FO88tXedYuefhGqZfLjVFHLcNKfJXM/98/wUAnQfg=";
    };
    date = "2026-01-19";
  };
  lineage_external_libdisplay_info_upstream = {
    pname = "lineage_external_libdisplay_info_upstream";
    version = "d24cede4ffdeb8a130d5923a04a315aa02a1de79";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_external_libdisplay-info-upstream.git";
      rev = "d24cede4ffdeb8a130d5923a04a315aa02a1de79";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-VXikD7CkMIGjGrSJOhveWtGbyM7Z7xf7VILH1FLXshY=";
    };
    date = "2026-01-19";
  };
  lineage_external_linux_firmware_mainline = {
    pname = "lineage_external_linux_firmware_mainline";
    version = "ef88d24dfc10e30826789b82fca417d6f913ee2a";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_external_linux-firmware-mainline.git";
      rev = "ef88d24dfc10e30826789b82fca417d6f913ee2a";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-ZpL3VF0nmER/tbNPC9KgGpZ37HOnTU7YeP7sdNcrC8E=";
    };
    date = "2026-06-05";
  };
  lineage_external_mesa = {
    pname = "lineage_external_mesa";
    version = "5a6bb76986d2d111599baf18e1dcd20b5ffd02c4";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_external_mesa.git";
      rev = "5a6bb76986d2d111599baf18e1dcd20b5ffd02c4";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-10u8XMhRt/pYWTUnXbSrnGQmfNrZckRkJ2OKUZDsCB0=";
    };
    date = "2026-09-02";
  };
  lineage_external_minigbm_upstream = {
    pname = "lineage_external_minigbm_upstream";
    version = "51b97430ef97474b386bc958d67e5171a86d0d76";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_external_minigbm-upstream.git";
      rev = "51b97430ef97474b386bc958d67e5171a86d0d76";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-/IbZg4nGYLyD4XVU1EOKNGiMaOu5aVJPSWMZvMVbngo=";
    };
    date = "2026-06-23";
  };
  lineage_gallery2 = {
    pname = "lineage_gallery2";
    version = "73803da5c066e0d6f67319d28087c965234b7850";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_packages_apps_Gallery2.git";
      rev = "73803da5c066e0d6f67319d28087c965234b7850";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-oOpYWQg27G8SJKtB8qujaVQLObw4ZMlTnE4khF/6J1w=";
    };
    date = "2026-05-17";
  };
  lineage_glimpse = {
    pname = "lineage_glimpse";
    version = "73cd3ffb1ec447a2280b7ce236f643aaf5663532";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_packages_apps_Glimpse.git";
      rev = "73cd3ffb1ec447a2280b7ce236f643aaf5663532";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-yfZ0SkaA7O7/Ir5WCO8O4U3WpU4xU+TV4nofAVxqLcM=";
    };
    date = "2026-09-01";
  };
  lineage_hardware_mainline_common = {
    pname = "lineage_hardware_mainline_common";
    version = "553ab9663b1a2f0afd8b95313f7701c29437be3e";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_hardware_mainline_common.git";
      rev = "553ab9663b1a2f0afd8b95313f7701c29437be3e";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-+GPpLO7OotRlCr29Xzuzo+Td784uahwSTvljZVL14mk=";
    };
    date = "2026-06-25";
  };
  lineage_kernel_mainline_configs = {
    pname = "lineage_kernel_mainline_configs";
    version = "1bf0c59fb857f4eadd02346f1dafedc4dbcdfafd";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_kernel_mainline_configs.git";
      rev = "1bf0c59fb857f4eadd02346f1dafedc4dbcdfafd";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-CLLB6UKMAdWafB4SOg3DNxmJmU4LYsW8r00VVJQ6apY=";
    };
    date = "2026-08-15";
  };
  lineage_kernel_virt_virtio = {
    pname = "lineage_kernel_virt_virtio";
    version = "f5d9986a0f480fc0887804c751b3155c0c4da06a";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_kernel_virt_virtio.git";
      rev = "f5d9986a0f480fc0887804c751b3155c0c4da06a";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-ZQowVhIcNZNCoIDt3P2kvgPAIshYsUZfH2GO+McFod0=";
    };
    date = "2025-12-31";
  };
  lineage_latinime = {
    pname = "lineage_latinime";
    version = "51e2f8ccf4c0e02836dccdd1a6d6ced4c363c0aa";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_packages_inputmethods_LatinIME.git";
      rev = "51e2f8ccf4c0e02836dccdd1a6d6ced4c363c0aa";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-55ZHH6unJ1hJcBjy0e1qHZMUkl6uTEmB3kd3BGMCZS8=";
    };
    date = "2026-09-01";
  };
  lineage_launcher3 = {
    pname = "lineage_launcher3";
    version = "15a56ccd09bbfa89f40b5afb84263841f400f0d3";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_packages_apps_Launcher3.git";
      rev = "15a56ccd09bbfa89f40b5afb84263841f400f0d3";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-HB4jGgI6gcjQMNNqaovHGNE/DBH6XLm1vpojKCR3O60=";
    };
    date = "2026-09-01";
  };
  lineage_launcher3_wip = {
    pname = "lineage_launcher3_wip";
    version = "15a56ccd09bbfa89f40b5afb84263841f400f0d3";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_packages_apps_Launcher3.git";
      rev = "15a56ccd09bbfa89f40b5afb84263841f400f0d3";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-HB4jGgI6gcjQMNNqaovHGNE/DBH6XLm1vpojKCR3O60=";
    };
    date = "2026-09-01";
  };
  lineage_prebuilts_bootmgr = {
    pname = "lineage_prebuilts_bootmgr";
    version = "811ba971e891b1f3e7412dfcaac633af5517892f";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_prebuilts_bootmgr.git";
      rev = "811ba971e891b1f3e7412dfcaac633af5517892f";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-zXwaM3Vf9L/xm8w/kpXKJpQBp5WDU6cY3arzGXnSj2g=";
    };
    date = "2026-02-18";
  };
  lineage_recorder = {
    pname = "lineage_recorder";
    version = "5db611be160bf28265ee952d52e2d433aedcafe8";
    src = fetchgit {
      url = "https://github.com/LineageOS/android_packages_apps_Recorder.git";
      rev = "5db611be160bf28265ee952d52e2d433aedcafe8";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-nc4sst9RksXTg7YVl5WWLpmg2bgzbmis7FX0bAynHlU=";
    };
    date = "2026-09-01";
  };
  morphe_apktool = {
    pname = "morphe_apktool";
    version = "04517bc7c687a6cfdcd813abb02e3134487baa95";
    src = fetchgit {
      url = "https://github.com/MorpheApp/Apktool.git";
      rev = "04517bc7c687a6cfdcd813abb02e3134487baa95";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-dgrjGXGJ86RDjBFB//1rOy7m1uxh3j3ZuXHRgRmLneQ=";
    };
    date = "2025-12-07";
  };
  morphe_multidexlib2 = {
    pname = "morphe_multidexlib2";
    version = "41cccc644cf1804362f7aa2fae96a6ffe67ffd22";
    src = fetchgit {
      url = "https://github.com/MorpheApp/multidexlib2.git";
      rev = "41cccc644cf1804362f7aa2fae96a6ffe67ffd22";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-NBubLnNjkZbGFlSCkwbTdvjDeYdRn4xJBJUNCCs/ccU=";
    };
    date = "2026-03-13";
  };
  sm8550_tab_device_gts9wifi = {
    pname = "sm8550_tab_device_gts9wifi";
    version = "9acd725f3dc46d5e88d913bfba04e5bb8a056759";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-tab/android_device_samsung_gts9wifi.git";
      rev = "9acd725f3dc46d5e88d913bfba04e5bb8a056759";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-AllU65KOUfwBsE95kxwqpC+EkHSyJyYfFegteIetwKY=";
    };
    date = "2026-05-15";
  };
  sm8550_tab_device_sm8550_common = {
    pname = "sm8550_tab_device_sm8550_common";
    version = "f8679e52f10e985bd2ceb9228760d8437634fcb7";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-tab/android_device_samsung_sm8550-common.git";
      rev = "f8679e52f10e985bd2ceb9228760d8437634fcb7";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-UstWyznDKIdrhxnBTSMt6ir4PVHoS3msmqs03znzpqs=";
    };
    date = "2026-05-15";
  };
  sm8550_tab_hardware_samsung = {
    pname = "sm8550_tab_hardware_samsung";
    version = "d893261eda11a0c7166d5af8cba639a9c081e7e1";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-tab/android_hardware_samsung.git";
      rev = "d893261eda11a0c7166d5af8cba639a9c081e7e1";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-50lnCmnsGJXtfll7vfzQ2d8Il6/S0vvktXPXbs0uh30=";
    };
    date = "2026-05-16";
  };
  sm8550_tab_kernel_sm8550 = {
    pname = "sm8550_tab_kernel_sm8550";
    version = "f52247296f8b456216fc908ac95e30c279fd0e7c";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-tab/android_kernel_samsung_sm8550.git";
      rev = "f52247296f8b456216fc908ac95e30c279fd0e7c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-1fbZR3Kf9iRMiavM0o5Ay595dUa50+1Urax9jodOXi4=";
    };
    date = "2026-05-15";
  };
  sm8550_tab_kernel_sm8550_modules = {
    pname = "sm8550_tab_kernel_sm8550_modules";
    version = "3df150495b0618371010cb7d03460a858cb62e3f";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-tab/android_kernel_samsung_sm8550-modules.git";
      rev = "3df150495b0618371010cb7d03460a858cb62e3f";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-J402e4AtE7M6q/lXb7VbzQ5Y1NUL6ZyyyDzvkNoi/lc=";
    };
    date = "2026-09-05";
  };
  sm8550_tab_vendor_gts9wifi = {
    pname = "sm8550_tab_vendor_gts9wifi";
    version = "bd0882d36594e1dbd772e14d9cdeb6c655718385";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-tab/android_vendor_samsung_gts9wifi.git";
      rev = "bd0882d36594e1dbd772e14d9cdeb6c655718385";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-b2pD44lvDgZkeNpzJI6GBal0GyAfHneBjVAFFWLtZ4Q=";
    };
    date = "2025-10-28";
  };
  sm8550_tab_vendor_sm8550_common = {
    pname = "sm8550_tab_vendor_sm8550_common";
    version = "baf2fdd19d0dae4e422da8002cf61143f1f1c325";
    src = fetchgit {
      url = "https://github.com/samsung-sm8550-tab/android_vendor_samsung_sm8550-common.git";
      rev = "baf2fdd19d0dae4e422da8002cf61143f1f1c325";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-xLNO4qmP48ct7huA7xja15r5XtQNfX267WWXpXTkTVI=";
    };
    date = "2025-10-28";
  };
  susfs414 = {
    pname = "susfs414";
    version = "77905b5a071e6f3669e3b1814cea30147c0801da";
    src = fetchgit {
      url = "https://gitlab.com/simonpunk/susfs4ksu.git";
      rev = "77905b5a071e6f3669e3b1814cea30147c0801da";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-cbQNeNp+sbm/6PH1KWCH5ggGy9KbGbzX7qUkhmc+Tdc=";
    };
    date = "2025-02-23";
  };
  susfs419 = {
    pname = "susfs419";
    version = "001e69919c6271f690fd00b17e4c721c9e599152";
    src = fetchgit {
      url = "https://gitlab.com/simonpunk/susfs4ksu.git";
      rev = "001e69919c6271f690fd00b17e4c721c9e599152";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-j4OIvgOE64nm67NyGsEnSRNygVVZaF5uhMx8ju+p6fQ=";
    };
    date = "2025-02-23";
  };
  termux_app = {
    pname = "termux_app";
    version = "4584488513c099f2e98bcfcd00f006d213248729";
    src = fetchgit {
      url = "https://github.com/termux/termux-app.git";
      rev = "4584488513c099f2e98bcfcd00f006d213248729";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-tAzCX1vXjnSZ/OE/c7XqJe4kNhBOSPt1pK4dkPvph0M=";
    };
    date = "2026-09-12";
  };
  termux_x11 = {
    pname = "termux_x11";
    version = "53f8437326dbbe26b3756480aece7f604d2c1900";
    src = fetchgit {
      url = "https://github.com/termux/termux-x11.git";
      rev = "53f8437326dbbe26b3756480aece7f604d2c1900";
      fetchSubmodules = true;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Ze9ZlUMV1nid8NFaHBnHzQMVEy31FRfxuvna++H4H38=";
    };
    date = "2026-09-11";
  };
  vendor_gapps15 = {
    pname = "vendor_gapps15";
    version = "af160b7fba89b2ea23aad432c7d0716e1639f0f9";
    src = fetchurl {
      url = "https://gitlab.com/MindTheGapps/vendor_gapps/-/archive/af160b7fba89b2ea23aad432c7d0716e1639f0f9/vendor_gapps-af160b7fba89b2ea23aad432c7d0716e1639f0f9.tar.gz";
      sha256 = "sha256-ZDPPagAlC/ECM4AdbYQOQX85sZq+JXzsDNLS3JCAud0=";
    };
    date = "2025-08-12";
  };
  vendor_gapps16 = {
    pname = "vendor_gapps16";
    version = "f8cdcffc2fb9181b8cc5a9b02d4b5908b64e5cdd";
    src = fetchurl {
      url = "https://gitlab.com/MindTheGapps/vendor_gapps/-/archive/f8cdcffc2fb9181b8cc5a9b02d4b5908b64e5cdd/vendor_gapps-f8cdcffc2fb9181b8cc5a9b02d4b5908b64e5cdd.tar.gz";
      sha256 = "sha256-g/kJGdrl59hZ8ojSD7eH/9nYzhf5u3VXjqUtrnPA7MU=";
    };
    date = "2026-06-12";
  };
  vendor_lindroid = {
    pname = "vendor_lindroid";
    version = "eadbb70f13209133d96a5355464388db8c6578de";
    src = fetchgit {
      url = "https://github.com/Linux-on-droid/vendor_lindroid.git";
      rev = "eadbb70f13209133d96a5355464388db8c6578de";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-hCzokst8gNIODntQgVIBbB/X7R9Rud71BdEIlyOEk3k=";
    };
    date = "2026-07-02";
  };
}
